...
try {
  ...
  await a.getByRole('button',{name:'关闭对话框',exact:true}).click()
  await a.locator('.page-actions').getByRole('button',{name:'新建项目',exact:true}).click()
  ...
  await b.getByRole('button',{name:'登录 / 注册',exact:true}).click(); await signIn(b)
  await b.getByRole('button',{name:'关闭对话框',exact:true}).click()
  await b.getByRole('button',{name:'打开我的账户',exact:true}).click()
  await b.locator('.account-menu').getByText('cloudtest',{exact:true}).waitFor()
  await b.getByRole('button',{name:'打开我的账户',exact:true}).click()
  await b.getByRole('button',{name:'云端项目',exact:true}).click()
  await b.locator('.cloud-list button').filter({ hasNotText: '尚未保存' }).first().click(); await b.getByRole('button',{name:'恢复为本地副本',exact:true}).click()
  ...
  await b.locator('.cloud-list button').filter({ hasNotText: '尚未保存' }).first().click(); await b.getByLabel(/恢复版本/).selectOption(first.binding.etag.slice(1,-1))
  await b.getByRole('button',{name:'恢复为本地副本',exact:true}).click();await b.waitForURL('**/editor/**');await ready(b)
  const historical=await cached(b); assert.deepEqual(historical.document,first.document);assert.ok(historical.binding);assert.notEqual(historical.binding.projectId,first.binding.projectId)
  await b.goto(`${base}/projects`);await b.getByRole('button',{name:'云端项目',exact:true}).click();await b.locator('.cloud-list button').filter({ hasNotText: '尚未保存' }).first().click()
